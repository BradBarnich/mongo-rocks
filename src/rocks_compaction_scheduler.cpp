...
public:
    explicit PrefixDeletingCompactionFilterFactory(const RocksCompactionScheduler* scheduler)
...
public:
    ...
    void scheduleCompactOp(const std::string& begin,
...
private:
    ...
    struct CompactOp {
        ...
        uint32_t _order;
        bool operator>(const CompactOp& other) const {
    ...
    Mutex _compactionMutex = MONGO_MAKE_LATCH();
...
CompactionBackgroundJob::~CompactionBackgroundJob() {
    ...
    // containing writes that have bypassed the WAL (writes issued with
    // WriteOptions::disableWAL=true) before shutting down background threads, so it's safe to be
    // called even if --nojournal mode is set.
...
void RocksCompactionScheduler::compact(const std::string& begin,
                                       const std::string& end,
                                       bool rangeDropped,
                                       uint32_t order) {
...
rocksdb::CompactionFilterFactory* RocksCompactionScheduler::createCompactionFilterFactory() const {
...
uint32_t RocksCompactionScheduler::loadDroppedPrefixes(rocksdb::Iterator* iter) {
    invariant(iter);
    const uint32_t rocksdbSkippedDeletionsInitial = (uint32_t)get_internal_delete_skipped_count();
    int dropped_count = 0;
...
Status RocksCompactionScheduler::dropPrefixesAtomic(const std::vector<std::string>& prefixesToDrop,
...
void RocksCompactionScheduler::notifyCompacted(const std::string& begin,
                                               const std::string& end,
                                               bool rangeDropped,
                                               bool opSucceeded) {
    if (rangeDropped) {
...
void RocksCompactionScheduler::droppedPrefixCompacted(const std::string& prefix, bool opSucceeded) {
    uint32_t int_prefix;
    bool ok = extractPrefix(prefix, &int_prefix);
    invariant(ok);
    {
    ...
    if (opSucceeded) {
        ...
        std::string ts;
        ts.append(sizeof(uint64_t), '\x00');
        rocksdb::Slice tsSlice(ts.data(), ts.size());
        syncOptions.timestamp = &tsSlice;
