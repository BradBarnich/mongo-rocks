...
TEST(RocksRecordStoreTest, Isolation1) {
...

    {
        ...
        ASSERT_OK(rs->updateRecord(t1.get(), loc1, "b", 2));
        ASSERT_OK(rs->updateRecord(t1.get(), loc2, "B", 2));

        // this should throw
        ASSERT_THROWS(rs->updateRecord(t2.get(), loc1, "c", 2).ignore(), WriteConflictException);
...
TEST(RocksRecordStoreTest, Isolation2) {
...

    {
...

        {
            ...
            ASSERT_THROWS(rs->updateRecord(t2.get(), loc1, "c", 2).ignore(),
...
TEST(RocksRecordStoreTest, OplogHackOnNonOplog) {
    ...
    BSONObj obj = BSON("ts" << Timestamp(2, -1));
    {
        WriteUnitOfWork wuow(opCtx.get());
        ASSERT_OK(
            rs->insertRecord(opCtx.get(), obj.objdata(), obj.objsize(), Timestamp()).getStatus());
        wuow.commit();
    }
...
TEST(RocksRecordStoreTest, CappedOrder) {
...

    {
        ...
        ServiceContext::UniqueOperationContext t1(harnessHelper->newOperationContext());
        std::unique_ptr<WriteUnitOfWork> w1(new WriteUnitOfWork(t1.get()));
        ASSERT_OK(rs->insertRecord(t1.get(), "b", 2, Timestamp()));
        // do not commit yet
        ...
        {  // create 2nd doc
            ...
            auto t2 = harnessHelper->newOperationContext(client2.get());
            {
                ...
                ASSERT_OK(rs->insertRecord(t2.get(), "c", 2, Timestamp()));
                w2.commit();
            }
        }
...
RecordId _oplogOrderInsertOplog(OperationContext* opCtx,
    ...
    ASSERT_OK(status);
    BSONObj obj = BSON("ts" << opTime);
    StatusWith<RecordId> res = rs->insertRecord(opCtx, obj.objdata(), obj.objsize(), Timestamp());
    ASSERT_OK(res.getStatus());
    return res.getValue();
}
