...
Status RocksEngine::createRecordStore(OperationContext* opCtx,
    ...
    if (s.isOK() && NamespaceString::oplog(ns)) {
        ...
        s = rocksToMongoStatus(_db->Put(writeOptions, encodedPrefix, rocksdb::Slice()));
    }
    return s;
}
...
Status RocksEngine::createSortedDataInterface(OperationContext* opCtx,
    ...
    RocksIndexBase::generateConfig(&configBuilder, _formatVersion, desc);
...
void RocksEngine::endBackup(OperationContext* opCtx) {
    _db->ContinueBackgroundWork();
}
...
rocksdb::Options RocksEngine::_options() const {
    ...
    options.compaction_filter_factory.reset(_compactionScheduler->createCompactionFilterFactory());
