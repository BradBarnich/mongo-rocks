...
CappedVisibilityManager::CappedVisibilityManager(RocksRecordStore* rs,
    ...
    if (_rs->_isOplog) {
        _oplogJournalThread =
            stdx::thread(&CappedVisibilityManager::oplogJournalThreadLoop, this, durabilityManger);
    }
}
...
void CappedVisibilityManager::oplogJournalThreadLoop(RocksDurabilityManager* durabilityManager) {
    try {
...
public:
    ...
    void insertKey(RocksRecoveryUnit* ru, const RecordId& loc, int len) {
        ...
        ru->Put(RocksRecordStore::_makePrefixedKey(_prefix, loc),
    ...
    void deleteKey(RocksRecoveryUnit* ru, const RecordId& loc) {
        ru->Delete(RocksRecordStore::_makePrefixedKey(_prefix, loc));
    ...
    void truncate(RocksRecoveryUnit* ru) {
        ru->TruncatePrefix(_prefix);
    }
...
RocksRecordStore::RocksRecordStore(StringData ns,
                                   StringData id,
                                   rocksdb::DB* db,
    ...
    : RecordStore(ns),
      ...
      _oplogKeyTracker(_isOplog ? new RocksOplogKeyTracker(rocksGetNextPrefix(_prefix)) : nullptr),
...
int64_t RocksRecordStore::storageSize(OperationContext* opCtx,
...
long long RocksRecordStore::dataSize(OperationContext* opCtx) const {
    RocksRecoveryUnit* ru = RocksRecoveryUnit::getRocksRecoveryUnit(opCtx);
    return _dataSize.load() + ru->getDeltaCounter(_dataSizeKey);
}
...
int64_t RocksRecordStore::cappedDeleteAsNeeded_inlock(OperationContext* opCtx,
    ...
    WriteUnitOfWork::RecoveryUnitState const realRUstate = opCtx->setRecoveryUnit(
        std::unique_ptr<RecoveryUnit>(realRecoveryUnit->newRocksRecoveryUnit()),
    ...
    try {
        ...
        while ((sizeSaved < sizeOverCap || docsRemoved < docsOverCap) && (docsRemoved < 20000) &&
            ...
            if (!_cappedVisibilityManager->isCappedHidden(newestOld)) {
                _cappedOldestKeyHint = newestOld;
            }
        ...
        if (docsRemoved > 0) {
            ...
            //     // we check if there's outstanding transaction that is older than
            //     // oldestAliveRecordId. If there is, we should not skip deleting that record next
            //     // time we clean up the capped collection. If there isn't, we know for certain
            //     this
            //     // is the record we'll start out deletions from next time
    ...
    if (_isOplog) {
        if ((_oplogSinceLastCompaction.minutes() >= kOplogCompactEveryMins) ||
            ...
            oldestAliveKey.append(sizeof(uint64_t), '\xff');
...
StatusWith<RecordId> RocksRecordStore::insertRecord(OperationContext* opCtx,
    ...
    if (_isOplog) {
        ...
        if (timestamp.isNull()) {
            timestamp = Timestamp(loc.repr());
            opCtx->recoveryUnit()->setOrderedCommit(false);
        }
    } else if (_isCapped) {
        loc = _cappedVisibilityManager->getNextAndAddUncommittedRecord(opCtx,
                                                                       [&]() { return _nextId(); });
    } else {
        loc = _nextId();
    }
    ...
    if (!timestamp.isNull()) {
        LOG(4) << "inserting record with timestamp " << timestamp;
...
Status RocksRecordStore::truncate(OperationContext* opCtx) {
    ...
    _changeNumRecords(opCtx, -_numRecords.load());
    _increaseDataSize(opCtx, -_dataSize.load());

    if (_isOplog) {
        _cappedVisibilityManager->setHighestSeen(RecordId());
        _oplogKeyTracker->truncate(ru);
    }
...
Status RocksRecordStore::oplogDiskLocRegister(OperationContext* opCtx,
...
void RocksRecordStore::updateStatsAfterRepair(OperationContext* opCtx,
 ...
 */
boost::optional<RecordId> RocksRecordStore::oplogStartHack(OperationContext* opCtx,
    ...
    auto key = _makeKey(startingPosition);
...
void RocksRecordStore::cappedTruncateAfter(OperationContext* opCtx, RecordId end, bool inclusive) {
    ...
    // Compute the number and associated sizes of the records to delete.
    {
        ...
        do {
            ...
            if (_isOplog) {
                _oplogKeyTracker->deleteKey(ru, record->id);
            }
...
RocksRecordStore::Cursor::Cursor(OperationContext* opCtx,
                                 rocksdb::DB* db,
                                 std::string prefix,
